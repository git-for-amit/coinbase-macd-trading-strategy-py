#!/bin/bash
# Create a minimal ta layer for AWS Lambda (Python 3.9 x86_64)

# Settings
TA_VERSION="0.11.0"  # Specify the ta version
PYTHON_VERSION="3.9"      # Python runtime version
TARGET_DIR="python/lib/python${PYTHON_VERSION}/site-packages"
LAYER_NAME="ta_minimal_layer"

# Create directories
mkdir -p ${TARGET_DIR}

# Install ta and its dependencies into target directory (minimal)
docker run --platform linux/amd64 --rm -v "$PWD":/var/task \
    public.ecr.aws/sam/build-python${PYTHON_VERSION}:latest \
    /bin/bash -c "pip install --no-cache-dir ta==${TA_VERSION} -t ${TARGET_DIR}"
